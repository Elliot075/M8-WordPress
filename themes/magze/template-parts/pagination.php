...
?>
<div class="magze-nav-pagination<?php echo ( $center_aligned_pagination ) ? ' center-aligned-pagination' : ''; ?>">
	<?php

	if ( 'button_click_load' == $pagination_type || 'infinite_scroll_load' == $pagination_type ) :

		magze_ajax_pagination( $pagination_type );

	elseif ( 'numeric' == $pagination_type ) :

		$prev_text        = sprintf(
			'<span class="nav-prev-text">%s</span>',
			magze_get_theme_svg('chevron-left')
		);
		$next_text        = sprintf(
			'<span class="nav-next-text">%s</span>',
			magze_get_theme_svg('chevron-right')
		);
