...
$posts_navigation_style = get_post_meta( $post->ID, 'magze_single_post_nav_style', true );
if ( empty( $posts_navigation_style ) ) {
	$posts_navigation_style = get_theme_mod( 'posts_navigation_style', 'style_1' );
}
...
if ( $next_post || $prev_post ) :
	...
	?>
	<nav class="navigation post-navigation<?php echo esc_attr( $pagination_classes ); ?>" aria-label="<?php esc_attr_e( 'Post', 'magze' ); ?>">
		<div class="magze-pagination-single <?php echo esc_attr( $posts_navigation_style ); ?> nav-links">
			<?php
			...
			switch ( $posts_navigation_style ) {
				...
				case 'style_3':
					...
					if ( 'style_2' == $posts_navigation_style ) {
						...
						$class      = ' magze-card-box img-animate-zoom magze-rounded-img';
						$overlay    = '';
					} else {
						$image_size = 'magze-large-img';
						$class      = ' img-animate-zoom magze-rounded-img';
						$overlay    = '<span aria-hidden="true" class="magze-block-overlay"></span>';
					}
					...
					if ( $prev_post ) :
						...
						<a class="nav-previous<?php if ( $prev_thumb ) : echo $class; endif; ?>" href="<?php echo esc_url( get_permalink( $prev_post->ID ) ); ?>">
							...
							<span class="post-info">
								<span class="meta-nav" aria-hidden="true"><?php esc_html_e( 'Previous Article', 'magze' ); ?></span> 
								<span class="screen-reader-text"><?php esc_html_e( 'Previous Post:', 'magze' ); ?></span> 
								<span class="title"><span class="title-inner magze-title-line"><?php echo wp_kses_post( get_the_title( $prev_post->ID ) ); ?></span></span>
							</span>
						</a>
						<?php
					endif;
					if ( $next_post ) :
						...
						<a class="nav-next<?php if ( $next_thumb ) : echo $class; endif; ?>" href="<?php echo esc_url( get_permalink( $next_post->ID ) ); ?>">
							...
							<span class="post-info">
								<span class="meta-nav" aria-hidden="true"><?php esc_html_e( 'Next Article', 'magze' ); ?></span> 
								<span class="screen-reader-text"><?php esc_html_e( 'Next Post:', 'magze' ); ?></span> 
								<span class="title"><span class="title-inner magze-title-line"><?php echo wp_kses_post( get_the_title( $next_post->ID ) ); ?></span></span>
							</span>
						</a>
						<?php
					endif;
