...
if ( $show_read_more ) {
	...
	if ( ! $read_more_text ) {
		$read_more_text = __( 'Read More', 'magze' );
	}
...
if ( in_array( 'category', $enabled_post_meta ) ) {
	$cats_enabled = true;
	$cat_style    = get_theme_mod( 'archive_category_style', 'style_5' );
	$cat_color    = get_theme_mod( 'archive_category_color_display', 'as_color' );
	$cat_limit    = get_theme_mod( 'archive_category_limit', 3 );
...
while ( $custom_archive_query->have_posts() ) :
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<div class="magze-article-block-wrapper<?php echo esc_attr( $class ); ?>">
			...
			<div class="entry-details">
				...
				<header class="entry-header">
					...
					if ( 'post' === get_post_type() ) :
						if ( $cats_enabled && 'before_title' == $cat_position ) :
							magze_post_categories( $cat_style, $cat_color, $cat_limit, $cat_label );
						endif;
					endif;
					?>
					<h2 class="entry-title color-accent-hover magze-limit-lines <?php echo esc_attr( $archive_title_limit ); ?>">
						<a href="<?php the_permalink(); ?>" class="magze-title-line"><?php the_title(); ?></a>
					</h2>
				...
				if ( 'post' === get_post_type() ) :
					...
					if ( $tags_enabled ) :
						magze_post_tags( $tag_style, $tag_limit, $tag_label );
					endif;
				endif;
