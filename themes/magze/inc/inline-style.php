...
if ( ! function_exists( 'magze_generate_font_family' ) ) :
	/**
	 ...
	 */
	function magze_generate_font_family( $font_base ) {
		if ( ! empty( $font_base ) ) {
...
if ( ! function_exists( 'magze_refactor_css' ) ) :
	/**
	 * Refactor CSS.
	 *
	 * @since 1.0.0
	 */
	function magze_refactor_css( $css ) {
		$refactored_css = str_replace( array( "\r\n", "\r", "\n", "\t", '  ', '    ', '    ' ), '', $css );
		return $refactored_css;
	}
endif;

if ( ! function_exists( 'magze_get_inline_css' ) ) :
	/**
	 * Outputs theme custom CSS.
	 *
	 * @since 1.0.0
	 */
	function magze_get_inline_css() {

		$css = '';

		$css .= magze_get_general_color_css();
		$css .= magze_get_global_elements_css();
		$css .= magze_get_general_widgetarea_css();
		$css .= magze_get_home_sections_css();
		$css .= magze_get_header_css();
		$css .= magze_get_footer_css();
		$css .= magze_get_typography_css();

		return magze_refactor_css( apply_filters( 'magze_global_inline_css', $css ) );
	}
endif;

if ( ! function_exists( 'magze_get_general_color_css' ) ) :
	function magze_get_general_color_css() {
		...
		return apply_filters( 'magze_general_color_css', $css );
	}
endif;

if ( ! function_exists( 'magze_get_global_elements_css' ) ) :
	function magze_get_global_elements_css() {
		...
		$offcanvas_menu_desc_color         = get_theme_mod( 'offcanvas_menu_desc_color', $defaults['offcanvas_menu_desc_color'] );
		...
		if ( $offcanvas_menu_desc_color !== $defaults['offcanvas_menu_desc_color'] ) :
			$css .= '
			:root {
				--global--color-offcanvas-menu-desc:' . esc_attr( $offcanvas_menu_desc_color ) . ';
			}';
		endif;
		...
		return apply_filters( 'magze_global_elements_css', $css );
	}
endif;

if ( ! function_exists( 'magze_get_general_widgetarea_css' ) ) :
	function magze_get_general_widgetarea_css() {
		...
		return apply_filters( 'magze_general_widgetarea_css', $css );
	}
endif;

if ( ! function_exists( 'magze_get_home_sections_css' ) ) :
	function magze_get_home_sections_css() {
		...
		if ( get_theme_mod( 'enable_banner' ) ) :
			...
			if ( get_theme_mod( 'enable_banner_arrows', true ) ) :
				...
				if ( $banner_arrows_bg_color !== $defaults['banner_arrows_bg_color'] ) :
					$banner_arrows_bg_color = magze_hex2rbga( $banner_arrows_bg_color, 0.8 );
		...
		return apply_filters( 'magze_home_sections_css', $css );
	}
endif;

if ( ! function_exists( 'magze_get_header_css' ) ) :
	function magze_get_header_css() {
		...
		if ( is_numeric( $header_padding_top ) ) :
			$css .= '
			.magze-site-header, .magze-site-header.has-header-image {
		...
		return apply_filters( 'magze_header_css', $css );
	}
endif;

if ( ! function_exists( 'magze_get_footer_css' ) ) :
	function magze_get_footer_css() {
		...
		return apply_filters( 'magze_footer_css', $css );
	}
endif;

if ( ! function_exists( 'magze_get_typography_css' ) ) :
	function magze_get_typography_css() {
		...
		if ( $primary_menu_font != $defaults['primary_menu_font'] ) :
			$css .= '
            :root {
                --font-primary-menu:' . sanitize_text_field( magze_generate_font_family( $primary_menu_font ) ) . ';
            }';
		endif;
		...
		if ( $sub_menu_font != $defaults['sub_menu_font'] ) :
			$css .= '
            :root {
                --font-sub-menu:' . sanitize_text_field( magze_generate_font_family( $sub_menu_font ) ) . ';
            }';
		endif;
		...
		if ( $primary_font != $defaults['primary_font'] ) :
			$css .= '
            :root {
                --font-headings:' . sanitize_text_field( magze_generate_font_family( $primary_font ) ) . ';
            }';
		endif;
		...
		if ( $secondary_font != $defaults['secondary_font'] ) :
			$css .= '
            :root {
                --font-base:' . sanitize_text_field( magze_generate_font_family( $secondary_font ) ) . ';
            }';
		endif;
		...
		return apply_filters( 'magze_typography_css', $css );
	}
endif;

if ( ! function_exists( 'magze_get_woo_inline_css' ) ) :
	/**
	 * Outputs woocommerce custom CSS.
	 *
	 * @since 1.0.0
	 */
	function magze_get_woo_inline_css() {
