...
class Magze_Grid_Posts extends Magze_Widget_Base {
	/**
	 * Constructor.
	 */
	public function __construct() {

		$this->widget_cssclass    = 'magze_grid_posts_widget';
		$this->widget_description = __( 'Displays posts in grid style', 'magze' );
		$this->widget_id          = 'magze_double_column_posts';
		$this->widget_name        = __( 'Magze: Grid Posts', 'magze' );
		$this->settings           = array(
			'title'                     => array(
				'type'  => 'text',
			...
			'card_layout'               => array(
				'type'    => 'select',
				'label'   => __( 'Card Layout', 'magze' ),
				'options' => array(
					''              => __( '&mdash; Select &mdash;', 'magze' ),
					'card_layout_1' => __( 'Layout One', 'magze' ),
					'card_layout_2' => __( 'Layout Two', 'magze' ),
				),
				'std'     => '',
			),
			...
			'hide_image'                => array(
				'type'  => 'checkbox',
				'label' => __( 'Hide Image', 'magze' ),
				'std'   => false,
			),
			'post_counter_position'     => array(
				'type'    => 'select',
				'label'   => __( 'Post Counter', 'magze' ),
				'options' => array(
					''             => __( '&mdash; Select &mdash;', 'magze' ),
	...
	public function get_posts( $args, $instance ) {
		...
		return new WP_Query( apply_filters( 'magze_grid_posts_query_args', $query_args ) );
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( ( $posts = $this->get_posts( $args, $instance ) ) && $posts->have_posts() ) {
			...
			do_action( 'magze_before_grid_posts' );
			...
			if ( 1 == $column ) {
				$col_class .= ' magze-grid-1';
			} elseif ( 2 == $column ) {
				$col_class .= ' row-cols-sm-2 magze-grid-2';
			} elseif ( 3 == $column ) {
				$col_class .= ' row-cols-md-3 magze-grid-3';
			} elseif ( 4 == $column ) {
				$col_class .= ' row-cols-sm-2 row-cols-xl-4 magze-grid-4';
			} elseif ( 5 == $column ) {
				$col_class .= ' row-cols-sm-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-5 magze-grid-5';
			} else {
				$col_class .= ' row-cols-sm-2 magze-grid-2';
			}
			...
			if ( $justify_content ) {
				$widget_class .= ' magze-justify-article-contents';
			}
			...
			<div class="magze-grid-posts-widget <?php echo esc_attr( $widget_class ); ?>">
			...
			do_action( 'magze_after_grid_posts' );
	...
	public function enqueue_assets() {
		magze_widget_css( $this->id_base, 'grid-posts' );
	}
}
