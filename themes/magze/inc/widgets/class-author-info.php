...
/**
 * Adds Magze_Author_Info widget.
 */
class Magze_Author_Info extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'magze_author_info_widget',
			esc_html__( 'Magze: Author Info', 'magze' ),
			array( 'description' => esc_html__( 'Displays author short info.', 'magze' ) )
		);

		$this->social_networks = apply_filters(
			'magze_author_widget_social_networks',
			array(
				'facebook',
				'twitter',
				'linkedin',
	...
	public function widget( $args, $instance ) {
		...
		$image_size = 'magze-medium-img';
		if ( $image_border_radius ) {
			$image_size = 'magze-square-img';
			$style     .= ' rd-visible';
		}
		...
		do_action( 'magze_before_author_info' );
		?>
		<div class="magze-author-info-widget <?php echo esc_attr( $wrapper_class ); ?>">
			<?php
				...
				if ( ! empty( $social_networks ) && is_array( $social_networks ) ) {
					...
					foreach ( $social_networks as $network ) {
						if ( ! empty( $instance[ $network ] ) ) {
							$svg = Magze_SVG_Icons::get_social_link_svg( $instance[ $network ] );
							if ( $svg ) {
								?>
								<li>
		...
		do_action( 'magze_after_author_info' );
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'author_pos' ) ); ?>">
				<?php esc_attr_e( 'Author Position:', 'magze' ); ?>
			</label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'author_desc' ) ); ?>">
				<?php esc_attr_e( 'Short Description:', 'magze' ); ?>
			</label>
		...
		<div>
			<label for="<?php echo esc_attr( $this->get_field_id( 'author_img' ) ); ?>">
				<?php esc_attr_e( 'Author Image:', 'magze' ); ?>
			</label>
			<?php
			...
			<div class="image-field">
				...
				<p>
					...
					<button type="button" class="upload_image_button button" data-uploader-title-txt="<?php esc_attr_e( 'Use Image', 'magze' ); ?>" data-uploader-btn-txt="<?php esc_attr_e( 'Choose an Image', 'magze' ); ?>">
						<?php esc_html_e( 'Upload/Add image', 'magze' ); ?>
					</button>
					<button type="button" class="remove_image_button button" style="<?php echo esc_attr( $remove_button_style ); ?>"><?php esc_html_e( 'Remove image', 'magze' ); ?></button>
				</p>
			</div>
		</div>
		<div>
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'image_border_radius' ) ); ?>">
					<?php esc_html_e( 'Enable Image Border Radius', 'magze' ); ?>
				</label>
			</p>
		</div>
		<div>
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'small_image' ) ); ?>">
					<?php esc_html_e( 'Enable Small Image Style', 'magze' ); ?>
				</label>
			</p>
		</div>
		<div>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'style' ) ); ?>">
					<span class="field-label"><?php esc_html_e( 'Author Info Style', 'magze' ); ?></span>
				</label>
				<select id="<?php echo esc_attr( $this->get_field_id( 'style' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'style' ) ); ?>" class="widefat">
					<option value="style_1" <?php selected( 'style_1', $style ); ?>><?php esc_html_e( 'Style 1', 'magze' ); ?></option>
					<option value="style_2" <?php selected( 'style_2', $style ); ?>><?php esc_html_e( 'Style 2', 'magze' ); ?></option>
					<option value="style_3" <?php selected( 'style_3', $style ); ?>><?php esc_html_e( 'Style 3', 'magze' ); ?></option>
				</select>
			</p>
		</div>
		<div>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'social_links_color' ) ); ?>">
					<span class="field-label"><?php esc_html_e( 'Social Links Color', 'magze' ); ?></span>
				</label>
				<select id="<?php echo esc_attr( $this->get_field_id( 'social_links_color' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'social_links_color' ) ); ?>" class="widefat">
					<option value="theme_color" <?php selected( 'theme_color', $social_links_color ); ?>><?php esc_html_e( 'Use Theme Color', 'magze' ); ?></option>
					<option value="brand_color" <?php selected( 'brand_color', $social_links_color ); ?>><?php esc_html_e( 'Use Brand Color', 'magze' ); ?></option>
				</select>
			</p>
		</div>
		<div>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'social_links_style' ) ); ?>">
					<span class="field-label"><?php esc_html_e( 'Social Links Style', 'magze' ); ?></span>
				</label>
				<select id="<?php echo esc_attr( $this->get_field_id( 'social_links_style' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'social_links_style' ) ); ?>" class="widefat">
					<?php
					$social_link_styles_arr = magze_get_social_links_styles();
		...
		<div>
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'inverted_block_color' ) ); ?>">
					<?php esc_html_e( 'Inverted Color', 'magze' ); ?>
				</label>
			</p>
			<em><?php esc_html_e( 'Can be used if you have dark background and want lighter color on the text.', 'magze' ); ?></em>
		</div>
		<?php
		$social_networks = $this->social_networks;
		if ( ! empty( $social_networks ) && is_array( $social_networks ) ) {
	...
	public function enqueue_assets() {
		magze_widget_css( $this->id_base, 'author-info' );
	}
}
